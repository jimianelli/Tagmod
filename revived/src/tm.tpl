 ...
 DATA_SECTION
  ...
  !! COUT(M_mu); COUT(M_cv); COUT(M_phase); COUT(lambda_M);
  init_number RF_mu;
  number ln_RF_mu;
	!! ln_RF_mu = log(RF_mu);
  init_number RF_cv;
  init_int ph_RF;
  !! COUT(RF_mu); COUT(RF_cv); COUT(ph_RF);
  !! COUT(RF_cv); COUT(ph_RF);
  number lambda_RF
  ...
  !! cout <<RF_idx<<endl; // nsim_days=nsim_days+1;
...
PARAMETER_SECTION
  ...
  init_bounded_number cht_repr(0.05,1,1);
	// init_vector RF(1,n_RF,ph_RF)
	init_number RF(ph_RF)
  number prob_no_tag;
  vector N(0,n_events);
...
PROCEDURE_SECTION
  ...
  // Derived outputs must be populated in every phase because REPORT_SECTION
  // is also evaluated after an ordinary maximum-posterior-density run.
  RepRate(1) = fsh_repr;
  RepRate(2) = cht_repr;
  ...
  if(mceval_phase())
  {
    if (mcflag)
    {
      mout<<Biomass<<" "<< Ninit<<" "<<ploss<<" "<<RepRate<<" "<<M<<" "<<ER<<" "<<surv<<" "<<obj_fun<<" "<<fcomp<<" "<<exp(RF)<<endl;
    }
    else
    {
      // mout<<"Biomass  N               p_loss     Rep_Rate_Fish  Rep_Rate_Charter     M      ER      Survival  ObjFun  ObjF_Tags ObjF_Repr ObjF_M ObjF_RF x x "; for (i=1;i<=n_RF; i++) mout<<" RF"<<i; mout<<endl;
      mout<<"Biomass  N               p_loss     Rep_Rate_Fish  Rep_Rate_Charter     M      ER      Survival  ObjFun  ObjF_Tags ObjF_Repr ObjF_M ObjF_RF x x RF" <<endl;
      mcflag=1;
    }
  }
...
// do_sims equals one for estimation, zero for catch simulations
  ...
  {
		if (i==1)
      N(i)            = N(i-1)*mfexp(-ndays(i)*M/365.) * mfexp(RF);
      //N(i)            = N(i-1)*mfexp(-ndays(i)*M/365.) * mfexp(RF(RF_idx(i)));
		else
...
// option to change penalty in different phases... this can be useful to detect local minima
  ...
  if (active(RF))
    fcomp(4)  = lambda_RF*square(RF-ln_RF_mu);
    // fcomp(4)  = lambda_RF*norm2(RF-ln_RF_mu);
  obj_fun  = sum(fcomp);
...
// Peterson estimates by fishing event
  for (i=1; i<=n_events; i++)
  {
    ...
    if((obs_tags(i))>0)
      ...
      //Ntot_Peterson(i) = (T * (1.-prob_no_tag) * surv * C(i) / RF(i) * rep + 1.) / (1+obs_tags(i)) -1 ;
...
// report section should be cleaned up!
  int i,ii;
  report<<"F "<<endl<<obj_fun<<" "<<fcomp<<endl;
  report<< "Ninit"<<endl;
  report<< Ninit  <<endl;
  report<< "Biomass"<<endl;
  report<< Biomass  <<endl;
...
//        Ntot_Peterson/1e6<<endl;
   ...
   report<<" RF est "<<RF<<endl<<" RF par "<<RF<<endl;
   if( num_events >0)
   {
...
GLOBALS_SECTION
  #include <admodel.h>
	...
	**/
	#undef COUT
	#define COUT(object) echoinput << #object "\n" << object << endl;
